...
- Post-match learning logs can now store post-match-only `game_state_profile`, `comeback_profile`, and `event_quality_score`.

### Changed
- Version source of truth is now `4.11.0-alpha`.
...
- FIFA official data is the first provider, but the storage and normalizer are provider-neutral for later StatsBomb/Wyscout/SofaScore-style adapters.
- V4.11 does not promote any model, alter production weights, or claim improved accuracy without future walk-forward paired evidence.

### Verified
- Backend test suite: `561 passed, 4 skipped`.
- Alembic head: `h8c9d0e1f2g3`.
